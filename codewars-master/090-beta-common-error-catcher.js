...
function checkLetters (word,letter,correctNumber=1){
  var res = new RegExp(`[^${letter}]${letter}{${correctNumber}}[^${letter}]`);
  return res.test(word);
}

// Alternative solution

function checkLetters(word, letter, correctNumber = 1) {
  if (correctNumber < 1 || correctNumber > 3) correctNumber = 1
  return word.split(letter).length - 1 === correctNumber
}