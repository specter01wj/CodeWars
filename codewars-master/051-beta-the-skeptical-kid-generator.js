...
function AlanAnnoyingKid(input){
  return `I don't think you ${input.slice(8,-1)} today, I think you ${input.slice(8,14) == 'didn\'t' ? `did ${input.match(/(?:\S+ ){3}(\S+)/)[1]} it!` : `didn\'t ${input.match(/(?:\S+ ){2}(\S+)/)[1].slice(0,-2)} at all!`}`
}