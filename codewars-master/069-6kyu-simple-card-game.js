...
function winner(deckSteve, deckJosh) {
  ...
  var countSteve = 0, countJosh = 0;
  for (var i = 0; i < deckSteve.length; i++) {
    if (priority.indexOf(deckSteve[i]) > priority.indexOf(deckJosh[i])) countSteve += 1;
    if (priority.indexOf(deckSteve[i]) < priority.indexOf(deckJosh[i])) countJosh += 1;
  }
  return countSteve > countJosh ? `Steve wins ${countSteve} to ${countJosh}`
    : (countSteve < countJosh ? `Josh wins ${countJosh} to ${countSteve}` : `Tie`);
}